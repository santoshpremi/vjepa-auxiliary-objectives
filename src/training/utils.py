...
def load_checkpoint(
    ...
    if hamiltonian_net is not None and "hamiltonian_net" in checkpoint:
        pretrained_dict = checkpoint["hamiltonian_net"]
        for k, v in hamiltonian_net.state_dict().items():
            if k not in pretrained_dict:
                logger.info(f'key "{k}" could not be found in loaded state dict')
            elif pretrained_dict[k].shape != v.shape:
                logger.info(
                ...
                pretrained_dict[k] = v
        msg = hamiltonian_net.load_state_dict(pretrained_dict, strict=False)
        logger.info(
            f"loaded pretrained hamiltonian net from epoch {epoch} with msg: {msg}"
        )
...
def init_opt(
    ...
    if hamiltonian_net is not None:
        param_groups.extend([
            {
                "params": (
                    p
                    for n, p in hamiltonian_net.named_parameters()
                    if ("bias" not in n) and (len(p.shape) != 1)
                )
            },
            {
        ...
        param_groups.extend([
            {
                "params": (
                    p
                    for n, p in dyn_head.named_parameters()
                    if ("bias" not in n) and (len(p.shape) != 1)
                )
            },
            {
