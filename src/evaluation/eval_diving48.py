...
def extract_features(data_list, encoder, device, num_frames, crop_size, base_dir):
...
def main():
    ...
    X_train, y_train = extract_features(train_data, encoder, device, num_frames, crop_size, base_dir)
    
    print("Extracting FULL validation features...")
    X_test, y_test = extract_features(test_data, encoder, device, num_frames, crop_size, base_dir)
    ...
    print(f"Training Attentive Probe (48 classes) for {args.epochs} epochs...")
    probe = AttentiveProbe(embed_dim=X_train.shape[-1], num_classes=48).to(device)
    ...
    print(f"\n========================================")
    print(f"[{args.name}] Diving-48 Action Recognition Accuracy: {acc*100:.2f}%")
    print(f"========================================")

    results = {
        "name": args.name,
        "dataset": "Diving-48 (V2)",
        "task": "frozen-encoder + attentive probe (48-class)",
